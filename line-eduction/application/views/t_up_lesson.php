    ...
    <div class="tpl-content-wrapper">
        <div class="lesson-container" style="width: 80%; margin: 0 auto">
            ...
            <form action="teacher/t_up" method="post" enctype="multipart/form-data">
                请选择上传课程: <select name="course">
                    <?php foreach ($results as $course){?>
                        <option value="<?php echo $course->cour_Id?>"><?php echo $course->cour_Name?></option>
                    <?php }?>
                </select>
                <input type="file" name="file" style="margin: 30px 0;width: 200px">
                <div  style="margin-top: -20px;color: red">视频名称只能由字母和数字组成,支持.mp4格式,大小不得超过128M</div>
                <input type="text" placeholder="请填写该视频的价钱" name="price" id="price">
                <span id="price_tip"></span>
                <input type="submit" value="提交" name="sub" style="display: block;margin-top: 20px">
            </form>
        </div>
    </div>
</div>
...
<script>
    $(function(){
        $('#price').on('blur',function(e,opts){
            var reg = new RegExp("^[0-9]*$");
            if(!reg.test(this.value)){
                $('#price_tip').html('请输入数字');
            }
            else
                $('#price_tip').html('');
            if(this.value<=0){
